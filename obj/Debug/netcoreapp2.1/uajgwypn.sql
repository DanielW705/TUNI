...
insert into @query select cu.ida, cu.idu from @query cu , rechazos re where cu.ida != re.idAlumno
else
return 'OK'
if((select COUNT(ac.idalumno) from @query cu , aceptados ac where cu.ida != ac.idAlumno)>1)
insert into @query select cu.ida, cu.idu from @query cu , aceptados ac where cu.ida != ac.idAlumno
else
return 'OK'
END
