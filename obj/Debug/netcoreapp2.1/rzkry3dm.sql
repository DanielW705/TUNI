...
if((select COUNT(re.idAlumno) from @query cu , rechazos re where cu.ida != re.idAlumno) > 1)
Begin
/*******/
insert into @query2 select cu.ida, cu.idu from @query cu , rechazos re where cu.ida != re.idAlumno
/*******/
END
IF((select COUNT(ac.idalumno) from @query cu , aceptados ac where cu.ida != ac.idAlumno)>1)
Begin
/*********/
insert into @query2 select cu.ida, cu.idu from @query cu , aceptados ac where cu.ida != ac.idAlumno
/******/
end
IF((select COUNT(cu.idA)from @query cu, solicitar sol where cu.idA != sol.idAlumno)>1)
BEGIN
insert into @query select cu.idA, cu.idu from @query cu, solicitar sol where cu.idA != sol.idAlumno
end
