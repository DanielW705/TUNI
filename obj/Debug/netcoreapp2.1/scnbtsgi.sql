...
CREATE TABLE [catCarreras] (
    ...
    [idArea] int NOT NULL,
    [carreraImpartida] nvarchar(max) NULL,
    CONSTRAINT [PK_catCarreras] PRIMARY KEY ([idCarrera]),
    CONSTRAINT [Relacion_areac_carrera] FOREIGN KEY ([idArea]) REFERENCES [catAreasCarrera] ([idArea]) ON DELETE CASCADE
);
...
CREATE TABLE [catalogoCarrerasT] (
    [idCarreTecnicas] int NOT NULL,
    [carreraTecnica] nvarchar(max) NULL,
    CONSTRAINT [PK_catalogoCarrerasT] PRIMARY KEY ([idCarreTecnicas]),
    CONSTRAINT [Relacion_carreratecnica_catalogodecarrerastecnicas] FOREIGN KEY ([idCarreTecnicas]) REFERENCES [carreraTecnicas] ([noderelcat]) ON DELETE CASCADE
);
...
CREATE UNIQUE INDEX [IX_catCarreras_idArea] ON [catCarreras] ([idArea]);
...
VALUES (N'20200514181132_1', N'2.1.14-servicing-32113');
